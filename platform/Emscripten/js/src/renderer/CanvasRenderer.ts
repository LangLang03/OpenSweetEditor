...
export class CanvasRenderer {
    ...
    private renderCurrentLine(model: EditorRenderModel): void {
    ...
    private renderSelectionRects(model: EditorRenderModel): void {
        const ctx = this.ctx;
    ...
    private renderLinkedEditingRects(model: EditorRenderModel): void {
        const ctx = this.ctx;
    ...
    private renderBracketHighlightRects(model: EditorRenderModel): void {
        const ctx = this.ctx;
    ...
    private renderGuideSegments(model: EditorRenderModel): void {
        const ctx = this.ctx;
        ...
        for (const segment of model.guideSegments) {
            ...
            if (segment.arrowEnd) {
                ...
                if (len > 0) {
                    ...
                    ctx.lineTo(
                        segment.end.x - arrowSize * (ux + uy * 0.5),
                        segment.end.y - arrowSize * (uy - ux * 0.5)
                    );
                    ctx.moveTo(segment.end.x, segment.end.y);
                    ctx.lineTo(
                        segment.end.x - arrowSize * (ux - uy * 0.5),
    ...
    private getGuideColor(type: number): string {
        switch (type) {
    ...
    private renderLines(model: EditorRenderModel): void {
        const ctx = this.ctx;
    ...
    private renderLine(ctx: CanvasRenderingContext2D, line: VisualLine): void {
        for (const run of line.runs) {
            this.renderRun(ctx, run);
        }
    }

    private renderRun(ctx: CanvasRenderingContext2D, run: VisualRun): void {
        switch (run.type) {
    ...
    private renderTextRun(ctx: CanvasRenderingContext2D, run: VisualRun): void {
    ...
    private renderWhitespaceRun(ctx: CanvasRenderingContext2D, run: VisualRun): void {
    ...
    private renderInlayHintRun(ctx: CanvasRenderingContext2D, run: VisualRun): void {
        if (run.text) {
            ...
            ctx.fillRect(
                run.x - padding - margin,
                run.y,
                run.width + padding * 2 + margin * 2,
                run.width > 0 ? run.width : 16
            );
    ...
    private renderPhantomTextRun(ctx: CanvasRenderingContext2D, run: VisualRun): void {
        ctx.font = this.getFont(run.style.fontStyle);
    ...
    private renderFoldPlaceholderRun(ctx: CanvasRenderingContext2D, run: VisualRun): void {
    ...
    private renderIcon(ctx: CanvasRenderingContext2D, x: number, y: number, _iconId: number, width: number, height: number): void {
        ctx.fillStyle = this.theme.defaultText;
        ctx.fillRect(x, y, width || 16, height || 16);
    }

    private renderDiagnostics(model: EditorRenderModel): void {
        const ctx = this.ctx;
    ...
    private getDiagnosticColor(severity: number): string {
        switch (severity) {
    ...
    private renderCompositionDecoration(model: EditorRenderModel): void {
        if (!model.compositionDecoration.active) return;
    ...
    private renderCursor(model: EditorRenderModel): void {
    ...
    private renderLineNumberSplit(model: EditorRenderModel): void {
        if (!model.splitLineVisible) return;
    ...
    private renderLineNumbers(model: EditorRenderModel): void {
        const ctx = this.ctx;
    ...
    private renderGutterIcons(model: EditorRenderModel): void {
        const ctx = this.ctx;
    ...
    private renderFoldMarkers(model: EditorRenderModel): void {
        const ctx = this.ctx;
    ...
    private renderScrollbars(model: EditorRenderModel): void {
        const ctx = this.ctx;
    ...
    private getFont(fontStyle: number): string {
        let style = '';
        if (fontStyle & 1) style += 'bold ';
        if (fontStyle & 2) style += 'italic ';
        if (fontStyle & 4) style += 'line-through ';
        return `${style}${this.fontSize}px ${this.fontFamily}`;
    }

    private getColor(argb: number): string {
        if (argb === 0) return '';
